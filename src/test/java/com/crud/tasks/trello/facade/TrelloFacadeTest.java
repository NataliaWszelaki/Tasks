...
class TrelloFacadeTest {
    ...
    void shouldCreateCard() {
        ...
        //Given
        TrelloCardDto trelloCardDto = new TrelloCardDto("Card", "Card's description", "top", "123qwe");
        TrelloCard trelloCard = new TrelloCard("Card", "Card's description", "top", "123qwe");

        when(trelloMapper.mapToCard(trelloCardDto)).thenReturn(trelloCard);
        when(trelloMapper.mapToCardDto(trelloCard)).thenReturn(trelloCardDto);
        when(trelloService.createTrelloCard(trelloCardDto)).thenReturn(new CreatedTrelloCardDto("123","New card", "https://test.test"));

        //When
        CreatedTrelloCardDto createdTrelloCardDto = trelloFacade.createCard(trelloCardDto);

        //Then
        assertEquals("New card", createdTrelloCardDto.getName());
    }
}